/*
 * ===================================================================
 * CANSAT EMISOR TODO-EN-UNO: BMP280 + LoRa EBYTE E32-433T30D PARA ESP32
 * ===================================================================
 * Lee el sensor BMP280 por I2C y transmite los datos por LoRa
 * usando el módulo EBYTE E32-433T30D (basado en UART).
 * 
 * En ESP32 se recomienda no usar SoftwareSerial, sino usar HardwareSerial 
 * (Serial1 o Serial2) ya que tiene múltiples UARTs por hardware.
 * 
 * Produce EXACTAMENTE el mismo formato de datos que el emisor DX-LR20
 * original: "CANSAT,id,temp,pres,alt,hum,ms"
 * 
 * HARDWARE:
 *   - ESP32 (cualquier variante estándar)
 *   - Sensor BMP280 (I2C)
 *   - Módulo LoRa EBYTE E32-433T30D (UART)
 * 
 * CONEXIONES BMP280 (I2C ESP32):
 *   VCC  -> 3.3V
 *   GND  -> GND
 *   SDA  -> GPIO 21
 *   SCL  -> GPIO 22
 ...
 *   VCC  -> 5V  o 3.3V (dependiendo de la alimentación del ESP32, el E32 soporta ambos)
 *   GND  -> GND
...
// Usamos HardwareSerial 2 en el ESP32
...
void esperarAUX() {
  ...
  while (digitalRead(E32_AUX_PIN) == LOW) {
    ...
    delay(10); // Alimentar el watchdog de ESP32
  }
...
void setup() {
  ...
  Serial.println(F("========================================="));
  Serial.println(F("  CANSAT EMISOR TODO-EN-UNO ESP32"));
  Serial.println(F("  BMP280 (I2C) + E32-433T30D (UART)"));
  Serial.println(F("========================================="));

  // ----- Inicializar Bus I2C ESP32 -----
  ...
  if (bmp.begin(0x76) || bmp.begin(0x77)) {
    ...
    Serial.println(F("ERROR! Sensor BMP280 no encontrado en I2C."));
  }
  ...
  // Inicializamos Serial2 (RX = 16, TX = 17 por defecto si no le pasamos pines se configuran así en ESP32)
  ...
  Serial.println(F("-----------------------------------------"));
  if (bmpOK) {
    Serial.println(F("[OK] Todo listo. Esperando comando START..."));
  } else {
...
void loop() {
  ...
  if (bmpOK) {
    ...
    alt  = bmp.readAltitude(1017);   // Presion ref nivel del mar aproximada
  }
